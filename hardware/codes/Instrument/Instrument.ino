...
const float MIN_VOLTAGE_V = 0.001;
const float MAX_POWER_MW = 5.0;
...
void setup()
{
  ...
  Serial.print(STAGE_CURRENTS_MA[0], 3);
  Serial.println(" mA");
}
...
void loop()
{
    ...
    if (isDigit(firstChar) || firstChar == '-' || firstChar == '.') {
      float incomingResistance = Serial.parseFloat();
      while (Serial.available()) {
        Serial.read();
      }
      if (!finalReported) {
        Serial.print("R=");
        Serial.println(incomingResistance, 5);
        handleResistanceUpdate(incomingResistance);
      }
      return;
    }
...
void streamContinuous_CSV(int posCh, int negCh) 
{
  ...
  while (!stopStream) 
  {
    ...
    // --- PRINT CSV FORMAT ---
    float current_mA = getStageCurrent_mA(currentStage);
    Serial.print(v, 8);
    Serial.print(",");
...
float getStageCurrent_mA(int stage) {
  if (stage < 0) {
    return STAGE_CURRENTS_MA[0];
  }
  if (stage > 3) {
    return STAGE_CURRENTS_MA[3];
  }
  return STAGE_CURRENTS_MA[stage];
}

void handleResistanceUpdate(float newResistance) {
  if (finalReported) {
    return;
  }
  ...
  float voltage_v = resistanceValue * current_A;
  float power_mW = voltage_v * current_A * 1000.0;

  if (stableCount >= STABLE_REQUIRED && voltage_v >= MIN_VOLTAGE_V && power_mW < MAX_POWER_MW) {
    Serial.println("*FINAL");
    Serial.print("FINAL,");
    Serial.print(voltage_v, 6);
    Serial.print(",");
    Serial.print(current_mA, 3);
    Serial.print(",");
    Serial.print(resistanceValue, 4);
    Serial.print(",");
...
void transitionToStage(int targetStage) {
  ...
  Serial.print(" -> ");
  Serial.print(getStageCurrent_mA(targetStage), 3);
  Serial.println(" mA");
}
