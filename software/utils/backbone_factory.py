...
def create_backbone(name: str, sim_config: Any, args: Any = None) -> object:
    ...
    if name == "stddev_window":
        return StdDevWindowBackbone(max(2, window_samples), sim_config.snapshot_std_threshold_v, min_stable_samples)

    if name == "baseline":
        return BaselineBackbone(max(2, window_samples), sim_config.snapshot_std_threshold_v, min_stable_samples)
    ...
    if name == "hysteresis":
        ...
        return HysteresisBackbone(max(1, window_samples), enter, exit_t, min_stable_samples)
